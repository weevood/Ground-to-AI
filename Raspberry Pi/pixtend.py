        ...
        try:
            ...
            if p.crc_header_in_error is False and p.crc_data_in_error is False:
                ...
                if cycle_counter > 10:
                    cycle_counter = 0
                    try:
                        log(TSDB_BUCKET, deviceName, influxdb_client.Point('climate').tag('room', 'technical').field('temperature', p.temp0))
                        log(TSDB_BUCKET, deviceName, influxdb_client.Point('climate').tag('room', 'technical').field('humidity', p.humid0))
